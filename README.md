...
```bash
ruby-serverless-chrome$ sam build
```
...
```bash
ruby-serverless-chrome$ sam local invoke HelloWorldFunction --event events/event.json
```
...
ruby-serverless-chrome$ curl http://localhost:3000/
```
...
```bash
ruby-serverless-chrome$ ruby tests/unit/test_handler.rb
```
